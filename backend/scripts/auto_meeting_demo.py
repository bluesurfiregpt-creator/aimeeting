...
【主持人 wrap_up 是关键信号】
  ...
  你的 consensus 输出 应该 直接整合 wrap_up 的要点,而不是独立"重新评估"."""
...
输出 JSON 单行,无其他文字:
{{
  "consensus": "<markdown 150-400 字.若主持人 wrap_up 已总结建议,直接整合;若无 wrap_up,从专家发言抽共同方向>",
  "dissents": [
    {{
      "point": "<分歧点 必须是 一个 具体决策,如 '是否放宽提取条件'>",
      "summary": "<具体 互斥立场 50-120 字,需明确两方各持什么态度>",
      "involved_agents": ["<专家名>", ...]
    }}
  ]
}}
...
提醒:
- 99% 的政务讨论里 各专家是互补关系,真分歧少见.dissents=[] 是合理的常态.
- 如果你 拿不准 是不是分歧 — **判定不是**.宁可漏识也不要 over-detect."""
