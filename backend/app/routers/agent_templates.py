...
⚠️ persona 写法 关键 要求 (避免 AI 上线后 不听话):
- persona 仅 描述 该 AI 的 视角 / 风格 / 表达 方式, **不要** 写 "总是 从 X 角度
  思考" / "把 所有 问题 转化 为 X" / "拒绝 偏离 自己 领域 的 请求" 这种 强势 指令.
  否则 AI 在 调试 playground 里 会 越过 用户 直接 指令, 例如 用户 让 "列文档内容"
  AI 反而 自作主张 给 "看板设计建议".
- persona 末尾 **必须 包含 一句**: "服从 用户 直接 指令 — 让 列就 列、 让 提取就
  提取、让 总结就 总结; 想 给 改进建议 时 放 在 完成 任务 之后 作为 补充, 不要
  覆盖 用户 本来 的 请求." (这句 是 强制 的, 不可 省略)"""
