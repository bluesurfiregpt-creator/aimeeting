...
G. **本实录已**剔除 **AI 专家** 的发言.如果你看到 "AI 专家"、"AI"、"RICE 框架"、
   "建议聚焦"、"专家观点" 等 类 AI 助手语言 — 这些 **不是** 真人说的,
   不能 作为「决策」/「待办」/「下一步」的依据.如果实录里 真的混了类似内容,
   忽略,只看 真人对话.
...
async def _build_named_transcript(db: AsyncSession, meeting_id: uuid.UUID) -> str:
    ...
    # v25.10 Bug B: 移除 agent messages 收集 — 不再传给 summary LLM
    # (它们在 docx 完整纪要的「AI 专家发言」单独节体现,但不混进纪要的「决策/待办」)
    ...
    # v25.10 Bug B: 客户测试反馈 AI 专家发言被纪要错误归为"决策 / 待办".
    # 移除 agent_msgs 不再传给 summary LLM.AI 发言仍在 docx 完整纪要里单独节体现
    # (build_minutes_docx 已实现),summary markdown 不混入.
    return "\n".join(lines)
