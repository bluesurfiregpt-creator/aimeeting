...
export default function AuthHeader() {
  ...
  return (
    <div className="fixed right-4 top-3 z-30 flex items-center gap-3 rounded-full border border-ink-700 bg-ink-900/90 px-3 py-1.5 backdrop-blur">
      <span className="text-xs text-zinc-500">
        {me.workspace_name}
      </span>
      <span className="text-zinc-700">·</span>
      <span className="text-xs text-zinc-300">{me.name}</span>
      <button
        onClick={logout}
