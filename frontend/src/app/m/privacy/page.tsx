 ...
 *   - 文字 zinc-* → ios.fg primary/secondary/tertiary
 *   - 链接 violet → systemPurple
 */
...
export default function PrivacyPolicyPage() {
  return (
    ...
    >
      ...
      <main
        className="space-y-5 p-5 pb-12 text-[15px] leading-relaxed"
        style={{ color: MR_COLORS.textSecondary }}
      >
        <p style={{ color: MR_COLORS.textTertiary }}>
          最近更新:{" "}
          <span style={{ color: MR_COLORS.textSecondary }}>
            2026 年 5 月
          </span>{" "}
          · 生效版本: v1.0
        </p>
        ...
        <Section title="一、引言">
          <p>
            欢迎使用 <strong style={{ color: MR_COLORS.textPrimary }}>智囊团</strong>(由 Aimeeting 团队 出品,
            以下称"本产品" 或"本服务"). 本指引用于说明 我们在 你 使用本产品 过程中
            收集 / 使用 / 存储 / 共享 / 保护 你个人信息 的方式. 请你 仔细阅读
            并 知悉.
          </p>
          <p>
            <strong style={{ color: MR_COLORS.textPrimary }}>
              使用本产品 视为 你 同意 本指引.
            </strong>{" "}
        ...
        <Section title="三、如何使用收集的信息">
          <ul className="ml-4 list-disc space-y-2">
            <li>提供 核心服务:实时转录 / AI 摘要 / 任务派发 / 知识沉淀 等.</li>
            <li>安全 + 反作弊:识别 异常登录 / 防止 账号被 盗用.</li>
            <li>
              产品 优化:基于 匿名化 的 聚合统计 (例如 全平台 总会议数 / 平均
              时长) 改进 功能, 不针对 个人.
            </li>
            <li>
              <Strong>
                我们 不会 把 你的 个人信息 用于 广告推送 / 出售 / 跨产品 追踪.
              </Strong>
            </li>
          </ul>
        </Section>
        ...
        <Section title="四、信息共享 与 第三方 SDK">
          <p>
            为 实现 本产品 功能, 我们 会 把 必要 的 信息 提供 给 以下 第三方
            服务, 严格 限于 完成 服务 所必需 的 范围:
          </p>
          <Table
        ...
        <Section title="五、信息存储 与 安全">
          <ul className="ml-4 list-disc space-y-2">
            ...
            <li>
              ...
              (你可 随时 在 应用 内 删除);操作日志 180 天; 错误日志 90 天.
            </li>
            <li>
              <Strong>传输 安全</Strong>:全站 强制 HTTPS / WSS, 通信 加密.
            </li>
            <li>
              <Strong>存储 安全</Strong>:数据库 + OSS 启用 静态加密; 管理员
              访问 走 内部 ABAC + 二次确认.
            </li>
          </ul>
        </Section>
        ...
        <Section title="六、你的权利">
          <ul className="ml-4 list-disc space-y-2">
            <li>
              <Strong>查阅</Strong>:你 可在 "我的 / 会议 / 任务" 等 页面 查阅
              自己 的 所有 数据.
            </li>
            <li>
            ...
            <li>
              <Strong>删除</Strong>:你 可 删除 自己 创建 的 会议 / 附件 / 声纹.
              删除 后 立即 从 业务库 移除, 备份 在 30 天 内 清除.
            </li>
            <li>
            ...
            <li>
              <Strong>撤回 授权</Strong>:你 可 在 "我的 / 设置" 内 关闭 麦克风
              / 声纹 等 单项 授权.
            </li>
            <li>
              <Strong>注销</Strong>:可 联系 工作区 owner 发起 账号 注销. 注销
              后 个人数据 彻底 删除, 不可恢复.
            </li>
          </ul>
        </Section>
        ...
        <Section title="七、未成年人保护">
          <p>
            本产品 面向{" "}
            <Strong>16 岁以上</Strong> 企业 / 政府 / 团队 用户 提供 服务. 我们
            不会 主动 收集 16 岁 以下 未成年人 的 个人信息. 若 你是 监护人, 且
            发现 未成年人 误注册, 请 联系 工作区 管理员 协助 注销.
          </p>
        </Section>

        <Section title="八、政策 变更">
          <p>
            我们 可能 不时 更新 本指引. 重大 变更 (例如 收集项 / 共享对象 发生
            实质 增加) 时, 会在 应用 内 通过 弹窗 显著 提示, 并 重新 征求 你的
            同意.
          </p>
        </Section>
        ...
        <Section title="九、联系我们">
          <p>
            如对 本指引 有 任何 疑问 / 投诉 / 维权 请求, 请 联系 工作区 owner /
            admin (在 "我的 / 设置" 内 可 查看 联系方式), 或 发送 邮件 至{" "}
            <a
        ...
        <div
          className="mt-8 rounded-2xl p-4 text-[13px]"
          style={{
        ...
        >
          本指引 解释权 归 智囊团 (Aimeeting) 团队 所有. 若 中文 与 任何 其他
          语种 版本 存在 歧义, 以 中文 版本 为准.
        </div>
      </main>
    </div>
  );
...
function Strong({ children }: { children: React.ReactNode }) {
  return <strong style={{ color: MR_COLORS.textPrimary }}>{children}</strong>;
}
  ...
  return (
    ...
    >
      <table className="w-full min-w-[480px] text-[12px]">
          ...
          >
            {cols.map((c, i) => (
              <th
                ...
                style={{ borderBottom: `0.5px solid ${MR_COLORS.hairline}` }}
              >
                {c}
              </th>
            ))}
        ...
        <tbody>
          {rows.map((row, r) => (
            <tr key={r} style={{ color: MR_COLORS.textSecondary }}>
              {row.map((cell, c) => (
                <td
                  key={c}
