...
> **目的**: Codex (或 任何 后续 AI agent / 真人) 接手 时 一站式 必读. 整合 `CLAUDE.md` (Claude 时期 的 守则) + `docs/NORTH_STAR.md` 关键 沉淀 + 实操 经验.
...
| 层 | 选型 | 关键 版本 |
|----|------|---------|
| **Frontend** | Next.js 15 (App Router) + React 19 + TypeScript 5 | `frontend/package.json` |
| **UI 风格** | 原生 CSS-in-JS (inline style) + tokens 模块 (W_TOKENS / MR_TOKENS / MR_COLORS), **不用 Tailwind / shadcn** | `frontend/src/components/web/tokens.ts` 等 |
| **Backend** | FastAPI + SQLAlchemy 2.0 (async) + asyncpg | `backend/requirements.txt`, fastapi 0.115 |
...
| **Deploy** | Docker compose + rsync + nginx + certbot | `deploy/*.sh` |
| **小程序** | 微信 原生 (无 vite, 用 微信 IDE 调试) | `wechat-miniprogram/` 浅色化 done, 编辑功能 不做 |

---

## 2. 目录 结构 说明

详 `HANDOFF.md` § 14. 关键提示:
...
| 端 | token | 文件 | 默认 theme |
|----|-------|------|----------|
| Web Workstation + 首页 | **W_TOKENS** (暗紫 双 theme) | `frontend/src/components/web/tokens.ts` | dark default |
| Web 会议室 (§ 7.1.1) | **MR_TOKENS** (双 theme) | `frontend/src/components/web/meeting-room/tokens.ts` | light default |
| Mobile + 小程序 | **MR_COLORS** (单 theme 浅) | `frontend/src/components/mobile/meeting-room/styles.ts` | 永远 light |
...
### 6.2 API 调用
- 全部 走 `frontend/src/lib/api.ts` 的 `api.xxx()` helper
...
- 模板 见 `docs/kimi-tests/v26.3-05-kimi.md`

### 7.2 backend AI 行为 → 双盲 (§ 8.7)
- Claude + Kimi 各 跑 同 剧本 → 客观 metric 对账
- 跑 `scripts/blind-test-runner.py` 或 `blind-test-chat-runner.py`
- 不一致 = 真问题, 一致 = 信任
...
- **不准** rsync `deploy/.env` / `backend/.env` (server-only, exclude 在 `rsync-up.sh`)
...
- `backend/app/demo_seed*.py` — workspace 数据
- `deploy/.env` / `backend/.env` — server-only
- `backend/app/auth.py` / `models.py` / `init_db.py` — schema + 权限 (改 必 PM 批 + migration)
- `frontend/src/lib/api.ts` 类型 (改 影响 全 frontend)
- `frontend/src/components/web/tokens.ts` / `meeting-room/tokens.ts` / `mobile/meeting-room/styles.ts` — 三 token 入口
...
1. **跟 NORTH_STAR 对齐** — `docs/NORTH_STAR.md` 是 产品 truth source
2. **跟 DESIGN_SYSTEM 对齐** — `docs/design/system/DESIGN_SYSTEM.md` 是 视觉 truth source
...
`CLAUDE.md` 是 Claude 时期 的 工作守则, 内容 大部分 跟 本 AGENTS.md 一致. 区别:
- `CLAUDE.md` 有 一些 历史 saga 经验 (e.g. v1.2.0 P1.2 折叠态 borrow dark token 是反例, 风格守门 由此 沉淀)
- 留 兼容, 不删. Codex 也 可以 看作 参考

如 `CLAUDE.md` 和 `AGENTS.md` 冲突, **以 `AGENTS.md` 为准** (更新).
